...
class Model:
    ...
    def __init__(
        self,
        win_to_draw_splitter: AVAILABLE_SPLITTERS,
        black_to_white_splitter: AVAILABLE_SPLITTERS,
        name: str,
    ) -> None:
    ...
    def fit(self, x: pd.DataFrame, y: pd.Series) -> None:
        ...
        win_to_draw_splitter_sample_weight = compute_sample_weight(
            "balanced", win_condition
        )
        self.win_to_draw_splitter.fit(
            x, win_condition, win_to_draw_splitter_sample_weight
        )

        # If the game is not a draw, predict which player wins
        black_to_white_sample_weight = compute_sample_weight(
            "balanced", y[win_condition]
        )
        self.black_to_white_splitter.fit(
            x[win_condition], y[win_condition], black_to_white_sample_weight
        )
    ...
    @staticmethod
    def _get_feature_importance(model: AVAILABLE_SPLITTERS):
    ...
    def estimate_model(
        self, x_train, y_train_data, x_valid, y_valid_data, **kwargs
    ) -> None:
        feature_importance_1 = self._get_feature_importance(self.win_to_draw_splitter)
        feature_importance_2 = self._get_feature_importance(
        ...
        print(" " * 4, "Overall:")
        estimate_baseline_model(
            self, None, x_train, y_train_data, x_valid, y_valid_data, **kwargs
        )
